...
public class Wallet {
    ...
    public void addTrade(TradeOrder tradeOrder){
        activePositions.computeIfAbsent(tradeOrder.getTicker(), k -> new ArrayList<>()).add(tradeOrder);
    }
    public void removeTrade(TradeOrder tradeOrder) throws IllegalArgumentException {
        String ticker = tradeOrder.getTicker();
        if(!activePositions.containsKey(ticker) || !activePositions.get(ticker).contains(tradeOrder)){
            throw new IllegalArgumentException("Wallet doesn't contain this TradeOrder");
        }
        activePositions.get(ticker).remove(tradeOrder);
    }
